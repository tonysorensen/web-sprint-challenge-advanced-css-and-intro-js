...
const fixName = artists[8]
fixName.name = "Vincent Van Gogh";
artists.splice(8,1, fixName);
...
function getArtistByIndex(id, artists) {
  ...
  const whichArtist = artists[id].name;
  console.log (whichArtist);
  return `The artist at index ${id} is ${whichArtist}`;
...
function lotsOfArt(artists){
  ...
  for(i=0; i<artists.length; i++){
    const howMany = artists[i].paintings;

    if(howMany>=100){
      prolificArray.push(artists[i].name);
    }
  }
    console.log(prolificArray);
    return(prolificArray);
